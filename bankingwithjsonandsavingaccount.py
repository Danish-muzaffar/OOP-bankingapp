...
class SavingAccount(Account):
    def __init__(self, *args, **kwargs):
        ...
        self.interest_rate = 0.03  # Example: 3%
...
class CurrentAccount(Account):
    def __init__(self, *args, **kwargs):
        ...
        self.overdraft_limit = 500  # e.g., $500
    ...
    def withdraw(self, amount):
        ...
        if self._Account__balance + self.overdraft_limit >= total:
            self._Account__balance -= total
            self._log_transaction("Withdrawal (Overdraft allowed)", amount, fee)
            print(f"Withdrew ${amount:.2f} with ${fee:.2f} fee. New balance: ${self._Account__balance:.2f}")
        else:
...
class Bank:
    def __init__(self):
        ...
        for acc_num, acc_data in data["accounts"].items():
            acc_type = acc_data.get("type", "Base")
            if acc_type == "Saving":
                account = SavingAccount(
                    ...
                    locked=acc_data.get("locked", False),
                    login_attempts=acc_data.get("login_attempts", 0),
                    pin_attempts=acc_data.get("pin_attempts", 0)
                )
    ...
    def create_account(self, name, username, password, pin, initial_balance):
        ...
        if acc_type == "saving":
            new_account = SavingAccount(name, username, password, pin, initial_balance)
        elif acc_type == "current":
            new_account = CurrentAccount(name, username, password, pin, initial_balance)
        else:
    ...
    def display_all_accounts(self):
        if not self.accounts:
            ...
            return
        account_list = [[acc.get_account_number(), acc.name, acc.username, acc.to_dict()["balance"],acc.to_dict().get("type", "Base"),"Locked" if acc.is_locked() else "Active"] for acc in self.accounts.values()]
        print(tabulate(account_list, headers=["Account Number", "Name", "Username", "Balance", "Type","Status"], tablefmt="grid"))

    def display_account_numbers_and_names(self):
        if not self.accounts:
            print("No accounts found.")
            return
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            bank.display_account_numbers_and_names()
        elif choice == "4":
            bank.admin_login()
        elif choice == "5":
